...
func NewSQLiteWatchlistRepository(dbSvc *DBService) *SQLiteWatchlistRepository {
	return &SQLiteWatchlistRepository{db: dbSvc.GetDB()}
}
...
// NewWatchlistService 构造函数，接受 DBService
func NewWatchlistService(dbSvc *DBService) *WatchlistService {
	// 迁移：使用 SQLite 仓库
	repo := NewSQLiteWatchlistRepository(dbSvc)
	
	// TODO: 迁移旧的 JSON 数据到 SQLite
